...
const Notification = ({}) => {
  ...
  const follow = async (ind) => {
    try {
      ...
      const { data : { data : { authenticated } } } = await axios.post(`http://localhost:3000/api/v1/users/follow/${username}`,{},{
        headers: {
    ...
    <div>
      ...
      {loading ?
        <div
          key={"2"}
          id="ideas"
          className="fixed lg:right-2 h-[calc(90vh-22px)] top-[calc(10vh+2vw)] lg:top-[calc(10vh+16px)] lg:w-[calc(100vw*(5.4/6.5))] w-100vw flex justify-center items-center gap-4 p-2 pb-0 flex-wrap overflow-scroll"
        >
        ...
        notifications.length > 0 ? 
          <div
            key={"1"}
            className="fixed lg:right-2 h-[calc(90vh-22px)] top-[calc(10vh+2vw)] lg:top-[calc(10vh+16px)] lg:w-[calc(100vw*(5.4/6.5))] w-full flex flex-col justify-start gap-4 p-2 pb-0 overflow-y-scroll overflow-x-hidden min-h-[calc(90vh)]"
          >
            <div className="flex-grow flex flex-col justify-start gap-4">
              {notifications.map(({
                ...
                <>
                  ...
                  <div
                    key={_id}
                    className="bg-[#797270] w-full flex justify-start items-center gap-4 rounded-xl px-2 py-1 text-md"
                  >
                    <img
                      src={profileImage}
                      alt="profile"
                    ...
                    <div>
                      {type == "follow" &&
                        <div className="flex-grow flex justify-between items-center gap-4">
                          ...
                          <button
                            onClick={() => follow(ind)}
                            className={`${isFollowing ? "bg-[#A7A7A9]" : "bg-[#C1EDCC]"} text-black rounded-lg p-1 hover:bg-[#B0C0BC]`}
                          >
                      ...
                      {type == "included" &&
                        <div className="flex-grow flex justify-between items-center gap-4">
                          ...
                          <button
                          onClick={() => openGroup(ind)}
                          className="bg-[#C1EDCC] text-black rounded-lg p-1"
                        >
                          See Group
                        </button>
                        </div>}
